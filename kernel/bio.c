...
#define BUCKETSIZE 13  // bucket的个数
#define BUFFERSIZE 5  // 每个bucket可用的buffer的个数

extern uint ticks;  // 系统时钟
...
binit(void) {
  for (int i = 0; i < BUCKETSIZE; i++) {
    initlock(&bcachebucket[i].lock, "bachebucket");
...
bget(uint dev, uint blockno)
{
  ...
  b = &bcachebucket[bucketno].buf[least_idx];
  b->dev = dev;
  b->blockno = blockno;
  b->lastuse = ticks;
  b->valid = 0;
